...
const JudgeDashboard = () => {
  ...
  useEffect(() => {
    getDashboardData("judge")
      .then((data) => setApiDashboard(data))
      .catch(() => {
  ...
  const displayedCases =
    apiDashboard?.cases?.length > 0
      ? apiDashboard.cases.map((c: any) => ({
          id: c.id,
          date: c.date || "",
          priority: c.priority || t.priority.normal,
          status: c.status || t.status.awaiting,
          title: c.title,
          type: c.type || "Civil",
          parties: c.parties || c.lawyer || "",
        }))
      : caseData;
