...
import { getAppointments } from "@/services/backend";
...
const Appointments = ({ lang = "en" }: AppointmentsProps) => {
  ...
  useEffect(() => {
    getAppointments("citizen")
      .then((data) => {
        if (Array.isArray(data.appointments)) {
          setApiAppointments(data.appointments);
        }
      })
      .catch(() => {
  ...
  const displayedUpcoming = normalizedAppointments.filter((a) => a.status !== t.status.completed);
  const displayedPast = normalizedAppointments.filter((a) => a.status === t.status.completed);
