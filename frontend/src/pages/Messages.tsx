...
import { getMessages } from "@/services/backend";
...
const Messages = ({ lang = "en" }: MessagesProps) => {
  ...
  useEffect(() => {
    getMessages("citizen")
      .then((data) => {
        if (Array.isArray(data.conversations)) setApiConversations(data.conversations);
      })
      .catch(() => {
          ...
          <div className="lg:col-span-2 bg-white rounded-xl border border-slate-200 flex flex-col">
            ...
            <div className="p-4 border-b flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="relative">
                  <img
                    src={displayedConversations[selectedChat]?.avatar || "/avatar/avatar.png"}
                    alt={displayedConversations[selectedChat]?.name || "Contact"}
                    className="w-10 h-10 rounded-full object-cover"
                  />
                  {displayedConversations[selectedChat]?.online && (
                    <div className="absolute bottom-0 right-0 w-3 h-3 bg-green-500 border-2 border-white rounded-full" />
                  )}
                </div>
                <div>
                  <h3 className="font-semibold text-slate-900">
                    {displayedConversations[selectedChat]?.name || "Contact"}
                  </h3>
                  <p className="text-xs text-slate-500">
                    {displayedConversations[selectedChat]?.online ? t.online : t.offline}
                  </p>
                </div>
              </div>
